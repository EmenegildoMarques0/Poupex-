...
export default async function DetailsLesson({ params }: DetailsCouseProps) {
        ...
        <div>
            <div className="flex items-center px-4 h-16 bg-neutral-50 dark:bg-neutral-900">
                <Link href="/courses" className="flex items-center gap-2">
                    <ArrowLeft />
                    <span className="first-letter:uppercase">{course.data.title}</span>
                ...
                <div className="min-w-[300px] space-y-3">
                    ...
                    <div>
                        {Array.from({ length: 20 }).map((_, idx) => {
                            const num = course.data.title.split("-")[2];
                            return num && (
