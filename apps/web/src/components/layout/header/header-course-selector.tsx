...
export const CourseSelector = memo(function CourseSelector({
  ...
  const filteredCourses = useMemo(
    () => courses.filter((course) => course.name.toLowerCase().includes(courseFilter.toLowerCase())),
      ...
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <Button variant="ghost" className="hidden gap-2 px-3 text-sm font-medium hover:bg-accent md:flex">
            <span className="max-w-[200px] truncate lg:max-w-[300px]">{currentCourse.name}</span>
            <ChevronDown className="h-4 w-4 opacity-50" />
          </Button>
        </DropdownMenuTrigger>
