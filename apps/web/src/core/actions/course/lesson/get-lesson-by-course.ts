...
export async function getLessonByCourseAction(id: string): Promise<ApiResponse<Lesson[]>> {
    if (!id) {
    ...
    try {
        ...
        const response = await fetch(`${API_URL}/api/v1/courses/${id}/lessons`, {
            method: "GET",
            headers: {
